...
var colors = ["red", "purple", "green", "blue", "orange", "yellow"], shadings = ["open", "striped", "solid"], shapes = ["oval", "diamond", "squiggle"], counts = [1, 2, 3], spinnings = ["no-spin", "slow-spin", "fast-spin"];

function makeStandardCard(card) {
  var count = counts[card.count], color = colors[card.color], shading = shadings[card.shading], shape = shapes[card.shape], spinning = spinnings[card.spinning];
  ...
  if (card.hidden) {
    ...
    var path = $$('path');
    path.addClass('shape').addClass(color).addClass(shading).addClass(spinning).attr("d", pth).appendTo(svgElem);
  }
  return svgElem;
}
...
function makeCard(card) {
  if (card.type == '3^4' || card.type == '3^5') {
