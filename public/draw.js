...
var colors = ["red", "purple", "green", "blue", "orange", "yellow"], shadings = ["open", "striped", "solid"], shapes = ["oval", "diamond", "squiggle"], counts = [1, 2, 3], spins = ["slow", "medium", "fast"];

function makeStandardCard(card) {
  var count = counts[card.count], color = colors[card.color], shading = shadings[card.shading], shape = shapes[card.shape], spin = spins[card.spin];
  var pth = "M" + placements[count] + " " + rep(paths[shape], count).join('m50,0 ');

  var svgElem = $$('svg');
  svgElem.addClass("card-svg").addClass("spin-" + spin).attr("viewbox", "0 0 200 100");
...
function makeCard(card) {
  if (card.type == '3^5' || card.type == '3^4') {
