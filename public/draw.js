...
var colors = ["red", "purple", "green", "blue", "orange", "yellow"], shadings = ["open", "striped", "solid"], shapes = ["oval", "diamond", "squiggle"], counts = [1, 2, 3], sizes = [0.7, 1.0, 1.3];

function makeStandardCard(card) {
  var count = counts[card.count], color = colors[card.color], shading = shadings[card.shading], shape = shapes[card.shape], size = sizes[card.size || 1];
  ...
  if (card.hidden) {
    ...
    path.attr("transform", "scale(" + size + ")");
  }
  return svgElem;
}
